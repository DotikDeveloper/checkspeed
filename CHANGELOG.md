...
- **улучшена обработка ошибок**: добавлена корректная обработка таймаутов и сетевых ошибок в `testUploadSpeed()`, неудачные попытки пропускаются без прерывания всего процесса измерения;
- **устойчивость к ошибкам**: если все попытки измерения upload провалились, функция возвращает 0 вместо ошибки, что предотвращает полный сбой тестирования скорости.
