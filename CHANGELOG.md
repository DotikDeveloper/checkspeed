...
- **добавление явных требований к версии Node.js**: добавлено поле `engines` в package.json с требованиями Node.js `>=20.19.0` для соответствия требованиям `jsdom` 27.4.0 и устранения потенциального конфликта версий с `lru-cache` 11.2.4 (который допускает Node.js 20.0.0+, тогда как jsdom требует >=20.19.0). Обновлён Dockerfile для использования Node.js LTS версии (`node:lts-alpine`) вместо Node.js 18 для соответствия новым требованиям и обеспечения долгосрочной поддержки.
